...
# Soft CTA shown at the end of local-simulator runs (terminal + HTML report).
LOCAL_CTA_TEXT = ("Don't want to run the Xcode simulator, or working on Linux / "
                  "Windows? Make these on a real cloud device with Revyl.")
LOCAL_CTA_URL = "https://revyl.ai"
...
_PREVIEW_STYLE = """<style>
  ...
  * { box-sizing: border-box; }
  ...
  .cta { display: inline-block; margin-top: 30px; max-width: 540px; text-decoration: none;
         color: #1c1833; background: #d8cffe; font-weight: 600; font-size: 13.5px;
         padding: 12px 18px; border-radius: 14px; line-height: 1.45; }
  .cta:hover { filter: brightness(1.04); }
  footer { margin-top: 28px; color: #6f6a93; font-size: 12.5px; text-align: center; }
</style>"""
...
def write_preview(out_dir, flow, outputs, loop_seconds, source="a device",
    ...
    cta_html = ""
    if cta and cta_url:
        cta_html = (f'\n  <a class="cta" href="{html.escape(cta_url, quote=True)}">'
                    f'{html.escape(cta)} →</a>')
    doc = f"""<!doctype html>
...
def main():
    ...
    preview = (write_preview(out_dir, flow, outputs, total / o["fps"], source=driver.source,
                             cta=driver.cta, cta_url=driver.cta_url) if outputs else None)
